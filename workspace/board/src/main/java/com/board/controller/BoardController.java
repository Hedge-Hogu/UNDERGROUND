...
public class BoardController {
	...
	public void getListPage(
			Model model, @RequestParam("num") int num) throws Exception {
		 ...
		 * 
		 * //한 페이지에 출력한 게시물 갯수 int postNum = 10;
		 * 
		 * //하단 페이징 번호([ 게시물 총 갯수 / 한페이지에 출력할 갯수 ]의 올림) int pageNum =
		 ...
		 * (int)(Math.ceil((double)num/pageNum_cnt)*pageNum_cnt);
		 * 
		 * // 표시되는 페이지 번호 중 첫번째 번호 int startPageNum = endPageNum - (pageNum_cnt -1);
		 * 
		 * // 마지막 번호 재계산 int endPageNum_tmp =
		 * (int)(Math.ceil((double)count/pageNum_cnt));
		 * 
	...
	public void getListPageSearch(
			Model model, @RequestParam("num") int num, 
						 @RequestParam(
								 name = "searchType", required = false, 
